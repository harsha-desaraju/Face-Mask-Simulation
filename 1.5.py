#exponential distribution for thread pitch
...
def exponential(upper,lower,scale):
    upper = upper/1e-6
    lower = lower/1e-6
    r = truncexpon(b=(upper-lower)/scale,loc=lower,scale=scale)
    return r.rvs()*1e-6

def generate_grid(lst1,lst2):
    temp1 = random.uniform(diameter_range[0],diameter_range[1])
    lst1.append(temp1)
    while(lst1[-1]<=total_length):
        temp2 = exponential(pitch_range[1],pitch_range[0],1)
        temp1 = random.uniform(min_or_same(diameter_range[0],temp2),diameter_range[1])
